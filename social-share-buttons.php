 ...
 * Plugin URI: https://zotabox.com/zbv2/socialbutton?utm_source=wordpress.com&utm_medium=Social%20Buttons&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * Description: Get more social share and follows with Social Sharing Buttons. Easy to customize style, size, color.
 * Version: 1.1.9
 * Author: Zotabox
 * Author URI: https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Social Buttons&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * License: SMB 1.0
 */
...
function zb_sb_admin_init(){
    ...
    /* Register js. */
	wp_register_script( 'zb_sb_admin_init', plugins_url('assets/js/main.js?v='.time(), __FILE__) );
	wp_enqueue_script('zb_sb_admin_init');
...
function zb_sb_show_admin_messages()
{
	...
	$message_intall = '<div id="message" class="updated fade">
		    ...
		    <a href="/wp-admin/admin.php?page=zb_sb">
	...
	if ( is_plugin_active( 'social-share-buttons/social-share-buttons.php') ) {
...
function zb_sb_admin_menu() {
	add_menu_page('Social Sharing Buttons', 'Social Buttons', 'administrator', 'zb_sb', 'zb_sb_setting',plugins_url( 'zotabox.png', __FILE__ ));
}

function zb_sb_setting(){
	$domain_action = 'https://zotabox.com';
	$token_key = wp_create_nonce('update_zb_sb_code');
	...
	if(isset($access_key) && !empty($access_key) && strlen($access_key) > 0 && $ztb_status_disconnect == $connected){
		...
		$button = '<a  target="zotabox" href="'.$domain_action.'/customer/access/PluginLogin/?customer='.$ztb_id.'&access='.$access_key.'&domain_secure_id='.$domain.'&app=sb&platform=wordpress&utm_source=wordpress.com">
			Configure your tools
		</a>';
		$form = '';
	}else{
		$form = '<form class="ztb-register-form" target="_blank" method="POST" action="'.$domain_action.'/customer/access/PluginAuth?app=sb&utm_source=wordpress.com&utm_medium=Social Buttons&utm_campaign=ecommerce%20plugins&token='.$token_key.'&platform=wordpress&access='.$access_key.'" id="account-info">
					...
					<div class="form-group button-wrapper">
						<input zb-plugin="zb_sb" class="ztb-submit-button" type="submit" value="Start Using Your New Tools Now" /><br><br>
	...
	<div class="ztb-wrapper">
		<div class="ztb-logo">
			<a href="https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Social Buttons&utm_campaign=ecommerce%20plugins&authuser=anonymous" title="Zotabox" target="zotabox">
...
function update_zb_sb_code(){
	if(isset($_REQUEST['token']) && wp_verify_nonce($_REQUEST['token'], 'update_zb_sb_code')){
		...
		if(!isset($domain) || empty($domain)){
			$redirect = admin_url('admin.php?page=zb_sb');
        	wp_safe_redirect($redirect);
		}else{
			if(wp_verify_nonce($_REQUEST['token'], 'update_zb_sb_code')){
